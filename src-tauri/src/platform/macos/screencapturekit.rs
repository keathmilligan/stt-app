...
//!
//! Implementation based on OmniRec's proven ScreenCaptureKit audio capture.
...
enum SCKCommand {
    Start { result_tx: mpsc::Sender<Result<(), String>> },
    Stop,
    Shutdown,
}
...
impl SCKAudioCapture {
    ...
    pub fn new() -> Result<Self, String> {
        ...
        println!("System Audio: Using ScreenCaptureKit (macOS 12.3+)");
...
fn run_sck_thread(
    ...
    println!("ScreenCaptureKit: Capture thread started");
    ...
    loop {
        ...
        match cmd_rx.recv_timeout(timeout) {
            ...
            Ok(SCKCommand::Stop) => {
                ...
                is_capturing.store(false, Ordering::SeqCst);
                println!("ScreenCaptureKit: Capture stopped");
            }
    ...
    println!("ScreenCaptureKit: Capture thread exiting");
}
...
impl UnsafeSCStreamError for AudioCaptureErrorHandler {
    fn handle_error(&self) {
        eprintln!("ScreenCaptureKit: Stream error occurred");
    }
}
    ...
    fn did_output_sample_buffer(&self, sample: Id<CMSampleBufferRef>, of_type: u8) {
        ...
        if !LOGGED_FORMAT.swap(true, Ordering::Relaxed) {
            println!(
                ...
                sample_rate, channel_count, asbd.format_flags, is_non_interleaved
            );
        }
        ...
        // Send to channel (use send - the receiving thread handles backpressure)
...
struct SCKCaptureState {
    ...
    // The stream runs in the background via the dispatch queue in add_stream_output
...
impl Drop for SCKCaptureState {
    fn drop(&mut self) {
        ...
        println!("ScreenCaptureKit: Cleaning up capture state");
    }
}
...
fn start_capture(audio_tx: mpsc::Sender<SCKAudioSamples>) -> Result<SCKCaptureState, String> {
    println!("ScreenCaptureKit: Starting capture");
    ...
    let stream_thread = std::thread::spawn(move || {
        ...
        println!("ScreenCaptureKit: Stopping stream");
